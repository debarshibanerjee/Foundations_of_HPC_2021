...
int main(int argc, char** argv) {
	...
	const size_t n = size1 * size2 * size3;
	...
	if (n % nproc != 0) {
		...
		if (rank == 0) {
			cout << "Sorry, exiting. The total size of matrix should be multiple of number of "
	...
	double matrix_1[size1][size2][size3];
	double matrix_2[size1][size2][size3];
	double matrix_3[size1][size2][size3];
	...
	for (int l = 0; l < ITER; ++l) {
		...
		if (rank == 0) {
			...
			/* cout << "Time to initialize the matrix on 1 thread = " << t_init_end - t_init_start
			 * << endl; */
		}
	...
	if (rank == 0) {
		/* cout << "Time to complete the program = " << endl; */
		cout << t_avg << endl;
		/* cout << "array 1" << endl; */
		/* size_t counter{0}; */
		/* for (size_t i = 0; i < size1; ++i) { */

...
function openmpi {
	cmd_gnu="mpirun -np 2 --report-bindings --map-by $1 --mca pml $2 --mca btl self,$3 --mca btl_$3_if_include $4 ./IMB-MPI1_gcc PingPong"
	for i in {1..10..1}; do
		echo "$1, $2, $3, $4 --- Run - $i"
...
function intelmpi {
	cmd_intel="mpiexec -np 2 -env I_MPI_DEBUG 5 -genv I_MPI_FABRICS=$1 -genv I_MPI_OFI_PROVIDER=$2 -genv I_MPI_PIN_PROCESSOR_LIST 0,$3 ./IMB-MPI1_intel PingPong"
	for i in {1..10..1}; do
		echo "$4, $1, $2 --- Run - $i"
...
function intelmpi_node {
	cmd_intel_node="mpiexec -np 2 -ppn=1 -env I_MPI_DEBUG 5 -genv I_MPI_FABRICS=$1 -genv I_MPI_OFI_PROVIDER=$2 ./IMB-MPI1_intel PingPong"
	for i in {1..10..1}; do
		echo "node, $1, $2 --- Run - $i"
		mkdir -p output_gpu/intelmpi_run_$i

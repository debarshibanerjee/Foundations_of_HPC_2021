...
for file in files:
    ...
    data[2] = (
        "# Calculated Latency = "
        + np.around(np.abs(latency_estimated), 3).astype(str)
        + " usec; Calculated Bandwidth = "
        + np.around(np.abs(bandwidth_estimated_sp), 3).astype(str)
        + " MBytes/sec \n"
    )
